...
if (isset($_POST['email'])) {
    ...
    if (is_uploaded_file($_FILES['foto']['tmp_name'])) {
        //entro aquí si el usaurio ha subido un archivo
        //Comprobamos que sea un archivo de imagen y NO exceda los 2MB
        if (!isImagenValida($_FILES['foto']['type'], $_FILES['foto']['size'])) {
            //o no es un archivo de imagen o excede los 2MB
            $errores = true;
        } else {
            //si estoy aqui el archivo es correcto en tamaño y tipo
            //le ponemos un nombre unico al archivo
            $nombreArchivo = "./img/" . uniqid() . "_" . $_FILES['foto']['name'];
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $nombreArchivo)) {
                $_SESSION['rutaImagen'] = $nombreArchivo;
            } else {
                $errores = true;
    ...
    //Si estoy aqui es pq todo ha ido bien, cargamos perfil.php para ve la imagen y el email
